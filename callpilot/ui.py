...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .nav a {{ display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 8px; color: #5f6963; font-size: 14px; font-weight: 750; }}
    .nav a span {{ width: 8px; height: 8px; border-radius: 3px; border: 1px solid var(--line-strong); background: #fff; }}
    .nav a:hover {{ background: #f1f5f3; color: var(--ink); }}
    .nav a.active {{ background: var(--accent-soft); color: var(--accent); }}
    .nav a.active span {{ border-color: var(--accent); background: var(--accent); }}
    .main {{ min-width: 0; }}
    .topbar {{ min-height: 70px; display: flex; justify-content: space-between; align-items: center; padding: 16px 28px; border-bottom: 1px solid var(--line); background: rgba(255,255,255,.78); backdrop-filter: blur(14px); }}
    ...
    .btn {{ border: 1px solid var(--line-strong); border-radius: 8px; background: #fff; color: var(--ink); padding: 10px 14px; min-height: 40px; font-weight: 800; cursor: pointer; display: inline-flex; align-items:center; justify-content:center; }}
    ...
    .temp-hot {{ background: var(--hot-soft); color: var(--hot); border-color: #f4c9d1; }}
    .temp-warm {{ background: var(--warm-soft); color: var(--warm); border-color: #f2d5a9; }}
    .temp-cold {{ background: #eef2f5; color: var(--cold); border-color: #dce4e9; }}
    .status-active, .status-won, .status-sent, .status-connected, .status-completed, .status-ready {{ background: var(--accent-soft); color: var(--accent); border-color: #cce7d7; }}
    .status-new, .status-demo, .status-requested, .status-draft, .status-queued {{ background: var(--blue-soft); color: var(--blue); border-color: #cbdaf7; }}
    .status-follow_up, .status-contacted {{ background: var(--warm-soft); color: var(--warm); border-color: #f2d5a9; }}
    .status-lost, .status-missing, .status-suppressed, .status-failed {{ background: var(--hot-soft); color: var(--hot); border-color: #f4c9d1; }}
    ...
    input, textarea, select {{ width:100%; border:1px solid var(--line-strong); border-radius:8px; padding:11px 12px; background:#fff; color:var(--ink); font:inherit; }}
    ...
    .bar {{ height:9px; border-radius:999px; overflow:hidden; background:#edf1ef; }}
    ...
    .scorebar-track {{ height:8px; border-radius:999px; overflow:hidden; background:#edf1ef; }}

...
def render_knowledge(query: dict[str, list[str]]) -> str:
    ...
    with db() as conn:
        ...
        results = search_knowledge(conn, selected, q) if selected and q else []
    ...
    doc_rows = "".join(
        f"""
        <tr>
          <td><strong>{esc(row['title'])}</strong><div class="muted">{esc(row['source_type'])} v{row['version']}</div></td>
    ...
    result_rows = "".join(
        ...
        <div class="item">
          ...
          <small>{esc(row.get('document_title') or row.get('source') or 'Knowledge base')} - {esc(row.get('category') or 'General')}</small>
        </div>
        """
        for row in results
    )
    ...
    <section class="grid two" style="margin-top:18px;">
      <form class="panel pad" method="post" action="/knowledge/ingest">
        ...
        <div class="form-grid" style="margin-top:14px;">
          ...
          <label class="full">Content<textarea name="content" rows="9" placeholder="Question | Answer | Category | Tags&#10;What hours are you open? | Monday to Friday 9 AM to 6 PM | Hours | hours,schedule"></textarea></label>
        </div>
...
def render_knowledge_document(document_id: int) -> str:
    ...
    <section class="panel table-wrap" style="margin-top:18px;">
      <table><thead><tr><th>Question</th><th>Answer</th><th>Tags</th><th>Status</th></tr></thead><tbody>{rows or '<tr><td colspan="4">No items attached.</td></tr>'}</tbody></table>
    </section>
    """

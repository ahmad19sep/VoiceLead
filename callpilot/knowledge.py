...
from .compliance import default_workspace_id
...
def parse_knowledge_content(content: str) -> list[dict[str, str]]:
    ...
    for idx, paragraph in enumerate(paragraphs, 1):
        ...
        if question and answer:
            items.append({"question": question, "answer": answer, "category": category, "tags": tags})
    return items
...
def ingest_knowledge_document(
    ...
    business = conn.execute("select id, workspace_id from businesses where id = ?", (business_id,)).fetchone()
    if not business:
        return {"success": False, "error": "Business not found."}
    workspace_id = business["workspace_id"] or default_workspace_id(conn)
    timestamp = now()
...
def get_knowledge_documents(conn: sqlite3.Connection, business_id: int | None = None) -> list[dict[str, Any]]:
    sql = """
    ...
    args: list[Any] = []
    if business_id:
        sql += " where knowledge_documents.business_id = ?"
        args.append(business_id)
    sql += " order by datetime(knowledge_documents.updated_at) desc, knowledge_documents.id desc"
    return [dict(row) for row in conn.execute(sql, args).fetchall()]


def get_knowledge_document(conn: sqlite3.Connection, document_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
...
def get_document_items(conn: sqlite3.Connection, document_id: int) -> list[dict[str, Any]]:
    rows = conn.execute("select * from knowledge_base where document_id = ? order by id", (document_id,)).fetchall()
    return [dict(row) for row in rows]


def search_knowledge(conn: sqlite3.Connection, business_id: int, query: str, limit: int = 12) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        ...
        where knowledge_base.business_id = ? and coalesce(knowledge_base.status, 'approved') = 'approved'
        """,
        (business_id,),
    ).fetchall()
    ...
    for row in rows:
        ...
        text = " ".join(
            ...
            for field in ["question", "answer", "category", "tags", "source", "document_title"]
        )
        score = len(terms & _words(text))
        if score:
            scored.append((score, item))
    scored.sort(key=lambda pair: (-pair[0], pair[1]["id"]))
    return [item | {"match_score": score} for score, item in scored[:limit]]


def knowledge_stats(conn: sqlite3.Connection, business_id: int | None = None) -> dict[str, int]:
    where = " where business_id = ?" if business_id else ""
    args = [business_id] if business_id else []
    docs = conn.execute(f"select count(*) from knowledge_documents{where}", args).fetchone()[0]
    items = conn.execute(f"select count(*) from knowledge_base{where}", args).fetchone()[0]
    approved = conn.execute(
        f"select count(*) from knowledge_base{where}{' and' if where else ' where'} coalesce(status, 'approved') = 'approved'",
        args,
    ).fetchone()[0]
    stale = conn.execute(
        f"""
        select count(*) from knowledge_documents
        {where}{' and' if where else ' where'} julianday('now') - julianday(coalesce(updated_at, created_at)) > ?
        """,
        [*args, STALE_DAYS],
    ).fetchone()[0]
